!function e(){let t=!1,n,r=!1,a=null,l=null,i=null,o=null,s=JSON.parse(localStorage.getItem("selectedItems"))||[],$={jenichg:"15.05.9726",zakladka:"15.05.9926",ФедорЛекс:"14.09.2026",Карасичек:"14.09.2026","Четвертая Рота":"20.08.2026"},d=[{name:"Энергетик Рэд (20 шт.)",selector:'a[href*="energydrink3:20"].buy_btn'},{name:"Энергетик Лайт (20 шт.)",selector:'a[href*="energydrink1:20"].buy_btn'},{name:"Энергетик Блэк (20 шт.)",selector:'a[href*="energydrink2:20"].buy_btn'},{name:"Бандана Ямакаси",selector:'a[href*="yamakasi_bandana"].buy_btn'},{name:"Майка Ямакаси",selector:'a[href*="yamakasi_shirt"].buy_btn'},{name:"Штаны Ямакаси",selector:'a[href*="yamakasi_pants"].buy_btn'},{name:"Золотая ксива",selector:'a[href*="zolotaya_ksiva"].buy_btn'},{name:"Кулон Хищника",selector:'a[href*="kulon_%D1%85ishnika"].buy_btn'},{name:"Пушка Хищника",selector:'a[href*="pushka_%D1%85ishnika"].buy_btn'},{name:"Кулон Хищник-2.0",selector:'a[href*="kulon_%D1%85ishnik_2"].buy_btn'}];function c(e){if(!e)return null;let t=e.split(".");if(3!==t.length)return null;let n=parseInt(t[0],10),r=parseInt(t[1],10)-1,a=parseInt(t[2],10);return new Date(a,r,n,23,59,59)}let p={intervals:{navigation:1e3,wheel:2e3,adTimer:100,autoBuy:1e4},selectors:{wheel:".vmmo-wheel",spinButton:"#wheel-button",adContent:'[data-fullscreen-element-name="ad-content"]',wheelLink:"#menu_wheel",activeSlider:'a.slider-link._active[href*="/cabinet/wheel"]',adTimer:'[data-fullscreen-element-name="timeout"]',adCloseButton:'[data-fullscreen-element-name="close-btn"]',cabinetLink:'a[href="/cabinet"]',headerLoginText:".header-login .login-text",menuShopLink:'a[href*="/cabinet/equipment/bratva/from/menu"]',shopPage:".equipment-list, .shop-content"},autoNavigateToWheel:"true"===localStorage.getItem("wheelAutoNavigate")},g={get:e=>document.querySelector(e),getAll:e=>document.querySelectorAll(e),exists:e=>!!document.querySelector(e),hasClass:(e,t)=>e?.classList?.contains(t),click:e=>!!e?.click&&(e.click(),!0),reload:()=>location.reload()};function u(){let e=g.get(p.selectors.headerLoginText);return e&&e.textContent.trim()?e.textContent.trim():null}function b(e){if(!e)return null;let t=$[e.toLowerCase()];return t?c(t):c($[e])}function f(){if(!l||!i)return!1;let e=new Date;return e<=i}function _(){return location.pathname.includes("/cabinet/wheel")}async function h(){if(!s.length){y();return}try{if(!(location.pathname.includes("/cabinet/equipment/bratva")&&!location.pathname.includes("/buy/"))){let e=g.get(p.selectors.menuShopLink);e&&(e.click(),await new Promise(e=>setTimeout(e,3e3)));return}for(let t of s){let n=s.some(e=>e.selector===t.selector);if(!n)continue;let r=g.get(t.selector);if(r&&!r.disabled&&null!==r.offsetParent){let a=window.getComputedStyle(r);if("0.5"!==a.opacity&&!r.disabled){r.click(),await new Promise(e=>setTimeout(e,4e3));let l=g.get(p.selectors.menuShopLink);l&&(l.click(),await new Promise(e=>setTimeout(e,3e3))),await new Promise(e=>setTimeout(e,3e3))}}}localStorage.setItem("autoBuyLastRun",Date.now().toString())}catch(i){let o=g.get(p.selectors.menuShopLink);o&&o.click(),localStorage.setItem("autoBuyLastRun",Date.now().toString())}}function x(){if(o&&clearInterval(o),0===s.length){alert("⚠️ Выберите хотя бы один предмет для автопокупки.");return}o=setInterval(h,p.intervals.autoBuy),localStorage.setItem("autoBuyEnabled","true"),localStorage.setItem("autoBuyLastRun",Date.now().toString()),m()}function y(){o&&(clearInterval(o),o=null,localStorage.setItem("autoBuyEnabled","false"),localStorage.removeItem("autoBuyLastRun"),m())}function m(){let e=document.getElementById("rental-info"),t=document.getElementById("script-status"),n=document.getElementById("start-btn"),a=document.getElementById("stop-btn"),s=document.getElementById("page-status"),$=document.getElementById("auto-nav-btn"),d=document.getElementById("auto-buy-btn");if(!e||!t||!s)return;let c=f(),g=function e(){if(!i)return"Не активировано";let t=new Date,n=i-t;return n<=0?"Срок истёк":`${Math.floor(n/864e5)}д ${Math.floor(n%864e5/36e5)}ч ${Math.floor(n%36e5/6e4)}м`}(),u=_();if(e.innerHTML=`
    ...
    `,h.appendChild(v);let w=document.createElement("div");w.id="page-status",w.style.cssText=`
      padding: 14px 16px;
      ...
      position: relative;
      overflow: hidden;
    `;let k=document.createElement("div");k.id="script-status",k.style.cssText=`
      padding: 14px 16px;
    ...
    `,d.forEach(e=>{let t=document.createElement("div");t.style.cssText=`
      ...
      `,n.addEventListener("change",()=>{n.checked?s.push(e):0===(s=s.filter(t=>t.selector!==e.selector)).length&&o&&y(),localStorage.setItem("selectedItems",JSON.stringify(s)),s.length>0&&!o&&x(),m()});let r=document.createElement("label");r.textContent=e.name,r.style.cssText=`
        ...
        color: #e8e8e8;
        cursor: pointer;
      `,t.appendChild(n),t.appendChild(r),I.appendChild(t)}),C.appendChild(T),C.appendChild(I);let S=document.createElement("div");S.style.cssText=`
      display: flex;
      gap: 12px;
      margin-bottom: 20px;
    `;let B=document.createElement("button");B.id="start-btn",B.innerHTML=`<span style="margin-right: 6px;">▶</span> <span>Запустить</span>`,B.style.cssText=`
      flex: 1;
      padding: 14px 20px;
      ...
      justify-content: center;
      letter-spacing: 0.3px;
    `,B.addEventListener("mouseenter",()=>{B.disabled||(B.style.background="linear-gradient(135deg, rgba(0, 255, 136, 0.25), rgba(0, 204, 106, 0.15))",B.style.borderColor="rgba(0, 255, 136, 0.5)",B.style.transform="translateY(-2px)",B.style.boxShadow="0 10px 30px rgba(0, 255, 136, 0.2)")}),B.addEventListener("mouseleave",()=>{B.disabled||(B.style.background="linear-gradient(135deg, rgba(0, 255, 136, 0.15), rgba(0, 204, 106, 0.1))",B.style.borderColor="rgba(0, 255, 136, 0.3)",B.style.transform="translateY(0)",B.style.boxShadow="none")});let z=document.createElement("button");z.id="stop-btn",z.innerHTML=`<span style="margin-right: 6px;">⏹</span> <span>Остановить</span>`,z.style.cssText=`
      flex: 1;
      padding: 14px 20px;
      ...
      justify-content: center;
      letter-spacing: 0.3px;
    `,z.addEventListener("mouseenter",()=>{z.disabled||(z.style.background="linear-gradient(135deg, rgba(255, 60, 60, 0.25), rgba(255, 60, 60, 0.15))",z.style.borderColor="rgba(255, 60, 60, 0.5)",z.style.transform="translateY(-2px)",z.style.boxShadow="0 10px 30px rgba(255, 60, 60, 0.2)")}),z.addEventListener("mouseleave",()=>{z.disabled||(z.style.background="linear-gradient(135deg, rgba(255, 60, 60, 0.15), rgba(255, 60, 60, 0.1))",z.style.borderColor="rgba(255, 60, 60, 0.3)",z.style.transform="translateY(0)",z.style.boxShadow="none")});let W=document.createElement("button");W.id="auto-nav-btn",W.style.cssText=`
      width: 100%;
      padding: 14px 16px;
    ...
    `;let M=document.createElement("span");M.innerHTML="\uD83D\uDE80",M.style.fontSize="14px";let H=document.createElement("span");H.textContent="АВТОЗАПУСК",H.style.fontWeight="500",W.appendChild(M),W.appendChild(H),W.addEventListener("click",()=>{p.autoNavigateToWheel=!p.autoNavigateToWheel,localStorage.setItem("wheelAutoNavigate",p.autoNavigateToWheel.toString()),m()}),W.addEventListener("mouseenter",()=>{p.autoNavigateToWheel||(W.style.background="linear-gradient(135deg, rgba(100, 100, 100, 0.3), rgba(70, 70, 70, 0.3))")}),W.addEventListener("mouseleave",()=>{p.autoNavigateToWheel||(W.style.background="linear-gradient(135deg, rgba(80, 80, 80, 0.2), rgba(50, 50, 50, 0.2))")});let N=document.createElement("button");N.id="auto-buy-btn",N.style.cssText=`
      ...
      letter-spacing: 0.2px;
      box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2);
    `;let A=document.createElement("span");A.innerHTML="\uD83D\uDED2",A.style.fontSize="14px";let R=document.createElement("span");R.textContent="АВТОПОКУПКА",R.style.fontWeight="500",N.appendChild(A),N.appendChild(R),N.addEventListener("click",()=>{if(0===s.length){alert("⚠️ Выберите хотя бы один предмет для автопокупки.");return}o?y():x(),m()}),N.addEventListener("mouseenter",()=>{o||(N.style.background="linear-gradient(135deg, rgba(100, 100, 100, 0.3), rgba(70, 70, 70, 0.3))")}),N.addEventListener("mouseleave",()=>{o||(N.style.background="linear-gradient(135deg, rgba(80, 80, 80, 0.2), rgba(50, 50, 50, 0.2))")});let Y=document.createElement("div");Y.id="progress-bar-container",Y.style.cssText=`
      width: 100%;
      height: 6px;
      border-radius: 3px;
      background: rgba(255, 255, 255, 0.1);
      ...
      overflow: hidden;
      position: relative;
    `;let q=document.createElement("div");q.id="progress-bar",q.style.cssText=`
      height: 100%;
      width: 0%;
      background: linear-gradient(90deg, #00ff88, #00cc6a);
      transition: width 0.2s ease-out;
      box-shadow: 0 0 10px rgba(0, 255, 136, 0.3);
    `,Y.appendChild(q),b.appendChild(Y),B.addEventListener("click",()=>l?f()?_()||p.autoNavigateToWheel?void(r||(E(),m())):alert("⚠️ Перейдите на страницу колеса."):alert("❌ Срок аренды истёк или вы не имеете доступа."):alert("❌ Ник не определён. Попробуйте перезагрузить страницу.")),z.addEventListener("click",()=>{r&&(L(),m())}),S.appendChild(B),S.appendChild(z),b.appendChild(h),b.appendChild(w),b.appendChild(k),b.appendChild(C),b.appendChild(N),b.appendChild(W),b.appendChild(S),a.appendChild(c),a.appendChild(b),document.body.appendChild(a),m();let j=document.getElementById("menu-content");function D(){let e="0px"===j.style.maxHeight||"0"===j.style.opacity;e?(j.style.maxHeight="800px",j.style.opacity="1",j.style.padding="20px",u.innerHTML="▲",localStorage.setItem("wheelMenuCollapsed","false")):(j.style.maxHeight="0",j.style.opacity="0",j.style.padding="0 20px",u.innerHTML="▼",localStorage.setItem("wheelMenuCollapsed","true"))}u.addEventListener("click",e=>{e.stopPropagation(),D()}),c.addEventListener("dblclick",e=>{e.target!==u&&D()});let P=!1,F=0,X=0;function U(e){if(e.target===u||e.target.closest("button")||e.target.closest("input"))return;e.preventDefault();let t=e.clientX||e.touches?.[0]?.clientX||0,n=e.clientY||e.touches?.[0]?.clientY||0;if(!t||!n)return;let r=a.getBoundingClientRect();F=t-r.left,X=n-r.top,P=!0,a.style.transition="none",document.body.style.userSelect="none",document.addEventListener("mousemove",G,{passive:!1}),document.addEventListener("touchmove",G,{passive:!1}),document.addEventListener("mouseup",J,{once:!0}),document.addEventListener("touchend",J,{once:!0})}function G(e){if(!P)return;e.preventDefault();let t=e.clientX||e.touches?.[0]?.clientX||0,n=e.clientY||e.touches?.[0]?.clientY||0;if(!t||!n)return;let r=t-F,l=n-X,i=window.innerWidth-a.offsetWidth,o=window.innerHeight-a.offsetHeight;a.style.left=Math.max(0,Math.min(r,i))+"px",a.style.top=Math.max(0,Math.min(l,o))+"px",a.style.right="auto"}function J(){P=!1,a.style.transition="all 0.4s cubic-bezier(0.4, 0, 0.2, 1)",document.body.style.userSelect="",document.removeEventListener("mousemove",G),document.removeEventListener("touchmove",G),document.removeEventListener("mouseup",J),document.removeEventListener("touchend",J);let e=a.getBoundingClientRect();localStorage.setItem("wheelMenuPosition",JSON.stringify({top:e.top,left:e.left}))}c.addEventListener("mousedown",U),c.addEventListener("touchstart",U,{passive:!1}),t&&(j.style.maxHeight="0",j.style.opacity="0",j.style.padding="0 20px",u.innerHTML="▼")}let w={handleWheel(){if(!r)return;let e=g.get(p.selectors.wheel),t=g.get(p.selectors.spinButton),n=g.exists(p.selectors.adContent);if(!n){if(e&&g.hasClass(e,"_lock")){let a=g.get(p.selectors.wheelLink);setTimeout(()=>g.click(a),0);return}if(t){setTimeout(()=>g.click(t),300);return}}},handleNavigation(){if(!r)return;let e=g.get(p.selectors.activeSlider);if(e){let t=g.get(p.selectors.wheelLink);setTimeout(()=>g.click(t),0)}},handleAdTimer(){if(!r)return;let e=g.get(p.selectors.adTimer),t=g.get(p.selectors.adCloseButton);!e&&t&&setTimeout(()=>g.click(t),0)}},k=function(){this.intervals=new Map,this.start=function(e,t,n){this.stop(e),this.intervals.set(e,setInterval(t,n))},this.stop=function(e){this.intervals.has(e)&&(clearInterval(this.intervals.get(e)),this.intervals.delete(e))},this.stopAll=function(){this.intervals.forEach(e=>clearInterval(e)),this.intervals.clear()}};function E(){!r&&(r=!0,n&&(n.start("navigation",()=>w.handleNavigation(),p.intervals.navigation),n.start("wheel",()=>w.handleWheel(),p.intervals.wheel),n.start("adTimer",()=>w.handleAdTimer(),p.intervals.adTimer)))}function L(){r&&(r=!1,n&&n.stopAll())}window.addEventListener("beforeunload",()=>{n&&n.stopAll()});try{t?n&&n.stopAll():(n=new k,t=!0),r=!1,l=null,i=null,(l=u())&&(i=b(l)),v(),function e(){let t="true"===localStorage.getItem("autoBuyEnabled");t&&s.length>0&&!o?x():(0===s.length||!t)&&o&&y()}(),r&&L(),setInterval(()=>{let e=u(),t=l&&f(),n=e&&b(e);e!==l&&(i=(l=e)?b(l):null,t&&!n&&(L(),y())),function e(){if(p.autoNavigateToWheel&&l&&f()&&!_()){let t=g.get(p.selectors.wheelLink);t&&(g.click(t),setTimeout(()=>{_()&&!r&&(E(),m())},2e3))}}(),document.getElementById("wheel-script-menu")?(m(),function e(){let t=document.getElementById("progress-bar");if(!t)return;let n=p.intervals.autoBuy,r=Date.now();if(!o){t.style.width="0%";return}let a=localStorage.getItem("autoBuyLastRun");if(!a){t.style.width="0%";return}let l=parseInt(a,10);if(isNaN(l)){t.style.width="0%";return}t.style.width=`${Math.min(100,(r-l)/n*100)}%`}()):v()},1)}catch(C){n&&n.stopAll()}}();
